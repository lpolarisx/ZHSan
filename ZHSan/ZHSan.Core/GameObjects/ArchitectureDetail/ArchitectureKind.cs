...
public class ArchitectureKind : GameObject
{
    ...
    public string HasAgricultureString => ToMark(HasAgriculture);

    public string HasCommerceString => ToMark(HasCommerce);

    public string HasTechnologyString => ToMark(HasTechnology);

    public string HasDominationString => ToMark(HasDomination);

    public string HasMoraleString => ToMark(HasMorale);

    public string HasEnduranceString => ToMark(HasEndurance);

    public string HasPopulationString => ToMark(HasPopulation);

    public string HasLongViewString => ToMark(HasLongView);

    public string HasObliqueViewString => ToMark(HasObliqueView);

    public string HasHarborString => ToMark(HasHarbor);

    private string ToMark(bool value) => value ? "○" : "×";
    ...
    public PlatformTexture Texture
    {
        get
        {
            if (texture == null)
            {
                texture = CacheManager.GetTempTexture("Content/Textures/Resources/Architecture/" + ID.ToString() + ".png");
            }
