    ...
    public class GameScenario
    {
        ...
        */
        ...
        public void ApplyTroopEvents()
        {
            ...
            foreach (TroopEvent event2 in TroopEvents)
            {
                if (TroopEventsToApply.TryGetValue(event2, out var list))
                {
        ...
        */
        ...
        private void InitializeTroopData()
        {
            ...
            foreach (TroopEvent event2 in TroopEvents)
            {
                ...
                {
                    event2.AfterHappenedEvent = TroopEvents.GetGameObject(event2.AfterEventHappened) as TroopEvent;
                }
            }
        }
...
//                ScenarioMap.TileWidth = 50;
            ...
            var routewayStore = new JsonStore<RoutewayConfig>(Path.Combine(dirPath, "Routeways.json"));
            var routeways = routewayStore.Load();
            Routeways = routeways.Select(x => new Routeway(x)).ToDictionary(x => x.ID);

            var troopStore = new JsonStore<TroopConfig>(Path.Combine(dirPath, "Troops.json"));
            var troops = troopStore.Load();
            Troops = troops.Select(x => new Troop(x)).ToDictionary(x => x.ID);

            var factionStore = new JsonStore<FactionConfig>(Path.Combine(dirPath, "Factions.json"));
            var factions = factionStore.Load();
            Factions = factions.Select(x => new Faction(x)).ToDictionary(x => x.ID);

            var captiveStore = new JsonStore<CaptiveConfig>(Path.Combine(dirPath, "Captives.json"));
            var captives = captiveStore.Load();
            captiveData = captives.Select(x => new Captive(x)).ToList();

            var scenarioStore = new JsonStore<GameScenarioConfig>(Path.Combine(dirPath, "GameScenarios.json"));
            var scenarios = scenarioStore.Load();
            var scenario = scenarios.FirstOrDefault();

            var diplomaticRelationStore = new JsonStore<DiplomaticRelationConfig>(Path.Combine(dirPath, "DiplomaticRelations.json"));
            var diplomaticRelations = diplomaticRelationStore.Load();
            ...
            {
                MOD = scenario.Mod;
                AiBattlingArchitectureStrings = scenario.AIBattlingArchitectureStrings;
                FatherIds = scenario.FatherIds;
                MotherIds = scenario.MotherIds;
                SpouseIds = scenario.SpouseIds;
                BrotherIds = scenario.BrotherIds;
                SuoshuIds = scenario.SuoshuIds;
                CloseIds = scenario.CloseIds;
                HatedIds = scenario.HatedIds;
                MarriageGranterId = scenario.MarriageGranterId;
                PlayerList = scenario.PlayerList;
                Date = new GameDate(scenario.Date);
            ...
            // scenarios = new List<GameScenarioConfig>
            // {
        ...
        public bool SaveGameScenario(string LoadedFileName, bool saveMap, bool saveCommonData, bool saveSettings, bool disposeMemory = true, bool fullPathProvided = false, bool editing = false)
        {
            ...
            foreach (YearTableEntry yt in YearTable)
            {
                yt.FactionsString = StaticMethods.SaveIdToString(yt.Factions);
            }
            ...
            diplomaticRelationStore.Save(diplomaticRelations);
        ...
        public YearTable getFactionYearTable(Faction faction)
        {
            var result = new YearTable();

            foreach (YearTableEntry i in YearTable)
            {
                if (i.IsGloballyKnown || i.Factions.Contains(faction) || Session.GlobalVariables.SkyEye)
                {
                    result.Add(i);
                }
            }
        ...
        public YearTable getFactionYearTableRecentYears(Faction faction, int y)
        {
            var result = new YearTable();
        ...
        public YearTable getOnlyFactionYearTable(Faction faction)
        {
            var result = new YearTable();
            foreach (YearTableEntry i in YearTable)
            {
                if (i.Factions.Contains(faction))
                {
                    result.Add(i);
                }
            }
