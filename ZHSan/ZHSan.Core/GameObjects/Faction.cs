...
#pragma warning restore CS0169 // The field 'Faction.AIThread' is never used
        public CombatStance BattleState = CombatStance.Peace;
        ...
        public void Init()
        {
            ...
            BattleState = CombatStance.Peace;
        ...
        public List<Legion> Legions { get; set; } = new();
        ...
        public List<Routeway> Routeways { get; set; } = new();
        ...
        public Faction(FactionConfig config)
        {
            ID = config.Id;
            Name = config.Name;
            LeaderID = config.LeaderID;
            ColorIndex = config.ColorIndex;
            CapitalID = config.CapitalID;
            TechniquePoint = config.TechniquePoint;
            TechniquePointForTechnique = config.TechniquePointForTechnique;
            TechniquePointForFacility = config.TechniquePointForFacility;
            Reputation = config.Reputation;
            SectionsString = config.SectionsString;
            InformationsString = config.InformationsString;
            ArchitecturesString = config.ArchitecturesString;
            TroopListString = config.TroopListString;
            RoutewaysString = config.RoutewaysString;
            LegionsString = config.LegionsString;
            BaseMilitaryKindsString = config.BaseMilitaryKindsString;
            UpgradingTechnique = config.UpgradingTechnique;
            UpgradingDaysLeft = config.UpgradingDaysLeft;
            AvailableTechniquesString = config.AvailableTechniquesString;
            PreferredTechniqueKinds = config.PreferredTechniqueKinds;
            PlanTechniqueString = config.PlanTechniqueString;
            AutoRefuse = config.AutoRefuse;
            chaotinggongxiandu = config.CourtContribution;
            guanjue = config.OfficialRank;
            IsAlien = config.IsAlien;
            NotPlayerSelectable = config.NotPlayerSelectable;
            PrinceID = config.PrinceID;
            YearOfficialLimit = config.YearOfficialLimit;
            GetGeneratorPersonCountString = config.GetGeneratorPersonCountString;
            TransferingMilitariesString = config.TransferingMilitariesString;
            MilitariesString = config.MilitariesString;
            ZhaoxianFailureCount = config.RecruitmentFailureCount;
            Destroyed = config.Destroyed;
            SecondTierXResidue = config.SecondTierXResidue;
            SecondTierYResidue = config.SecondTierYResidue;
            ThirdTierXResidue = config.ThirdTierXResidue;
            ThirdTierYResidue = config.ThirdTierYResidue;
        }
        
        public FactionConfig ToConfig()
        {
            return new FactionConfig
            {
                ...
                CapitalID = CapitalID,
                TechniquePoint = TechniquePoint,
                TechniquePointForTechnique = TechniquePointForTechnique,
                TechniquePointForFacility = TechniquePointForFacility,
                Reputation = Reputation,
                SectionsString = SectionsString,
                InformationsString = InformationsString,
                ArchitecturesString = ArchitecturesString,
                TroopListString = TroopListString,
                RoutewaysString = RoutewaysString,
                LegionsString = LegionsString,
                BaseMilitaryKindsString = BaseMilitaryKindsString,
                UpgradingTechnique = UpgradingTechnique,
                UpgradingDaysLeft = UpgradingDaysLeft,
                AvailableTechniquesString = AvailableTechniquesString,
                PreferredTechniqueKinds = PreferredTechniqueKinds,
                PlanTechniqueString = PlanTechniqueString,
                AutoRefuse = AutoRefuse,
                CourtContribution = chaotinggongxiandu,
                OfficialRank = guanjue,
                IsAlien = IsAlien,
                NotPlayerSelectable = NotPlayerSelectable,
                PrinceID = PrinceID,
                YearOfficialLimit = YearOfficialLimit,
                GetGeneratorPersonCountString = GetGeneratorPersonCountString,
                TransferingMilitariesString = TransferingMilitariesString,
                MilitariesString = MilitariesString,
                RecruitmentFailureCount = ZhaoxianFailureCount,
                Destroyed = Destroyed,
        ...
        public PersonList Persons
        {
            get
            {
                ...
                foreach (var architecture in Architectures)
                {
                    ...
                    foreach (var person in architecture.GetMovingPersons())
                    {
                        result.Add(person);
                    }
                }
        ...
        */
        ...
        /// 获取势力所有宝物（君主除外）
        /// </summary>
        public List<Treasure> GetAllTreasuresExceptLeader()
        {
        ...
        public List<Point> GetAllKnownArea()
        {
            ...
            foreach (var point in knownAreaData.Keys)
            {
                var informationLevel = GetKnownAreaData(point);
                if (informationLevel != InformationLevel.None && informationLevel != InformationLevel.Unknown)
                {
                    result.Add(point);
                }
            }
        ...
        private List<Troop> visibleTroopsCache = null;
        public List<Troop> GetVisibleTroops()
        {
            if (visibleTroopsCache != null) return visibleTroopsCache;
            
            var result = new List<Troop>();
            foreach (var point in GetAllKnownArea())
            {
        ...
        public void InitLegions(List<Legion> legions)
        {
        ...
        public void InitRouteways(List<Routeway> routeways)
        {
        ...
        public int TotalFeiziCount()
        {
            ...
            foreach (var architecture in Architectures)
            {
                count += architecture.GetConcubines().Count;
            }
            return count;
        }
        ...
        private void AIHouGong()
        {
                ...
                if (a.GetConcubines().Count > 0)
                {
                    ...
                    foreach (var b in Architectures)
                    {
                        if (!b.withoutTruceFrontline 
                            ...
                            && (b.Meinvkongjian > b.GetConcubines().Count || b.BelongedFaction.IsAlien)
                            && b.Endurance > maxPop)
                        {
                            maxPop = b.Endurance;
                            dest = b;
                    ...
                    if (dest != null)
                    {
                        var concubines = a.GetConcubines();
                        concubines.Sort((a, b) => b.Merit.CompareTo(a.Merit));

                        int cnt = dest.BelongedFaction.IsAlien ? 9999 : dest.Meinvkongjian - dest.GetConcubines().Count;
                        int moved = 0;

                        foreach (var person in concubines)
                        {
                            if (person.ArrivingDays <= 0 && person.LocationArchitecture != dest)
                            {
                                person.MoveToArchitecture(dest);
                                moved++;
                                if (moved >= cnt) break;
                            }
                        }
            ...
            if (GameObject.Random(10) == 0)
            {
                ...
                foreach (var a in Architectures)
                {
                    foreach (var p in a.GetReleasableConcubines())
                    {
            ...
            if (hougongValid)
            {
                ...
                {
                    if ((this.Leader.LocationArchitecture.Meinvkongjian - this.Leader.LocationArchitecture.GetConcubines().Count <= 0 && !this.IsAlien) ||
                ...
                {
                    Architecture dest = null;
                    if ((this.Leader.LocationArchitecture.Meinvkongjian - this.Leader.LocationArchitecture.GetConcubines().Count > 0 &&
                    ...
                    else
                    {
                        foreach (var a in Architectures)
                        {
                            if (((a.Meinvkongjian - a.GetConcubines().Count > 0 && a.Fund >= Session.Parameters.NafeiCost + a.EnoughFund) || this.IsAlien)
                    ...
                    if (dest != null)
                    {
                        ...
                        foreach (var architecture in Architectures)
                        {
                            foreach (var person in architecture.GetFemaleWarriors())
                            {
                                if (!(Leader.isLegalFeiZiExcludeAge(person) && person.isLegalFeiZiExcludeAge(Leader))) continue;

                                if (IsPersonForHouGong(person, person.IsCaptive) 
                                    && person.WaitForFeiZi == null 
                                    && person.BelongedArchitecture != null)
                                {
                        ...
                        candidate.Sort((a, b) => b.UntiredMerit.CompareTo(a.UntiredMerit));
                    ...
                    !this.Leader.huaiyun && this.Leader.WaitForFeiZi == null)
                {
                    if (hougongValid)
                    {
                        if (GameObject.Random((int) (60f / (this.Leader.Ambition + 1) * Math.Sqrt(this.Leader.NumberOfChildren))) == 0)
                        {
                            ...
                            foreach (var a in Architectures)
                            {
                                foreach (var p in a.GetNotPregnantConcubines())
                                {
                    ...
                    else
                    {
                        if (GameObject.Random((20 + this.Leader.NumberOfMaleChildren * 20) / (this.Leader.Ambition * this.Leader.Ambition + 1)) == 0)
                        {
                            Person target = null;
                            float max = 0;
                            foreach (var p in Leader.LocationArchitecture.GetNotPregnantConcubines())
                            {
                                if (p.huaiyun) continue;
        ...
        {
            if (Leader.Status == PersonStatus.Captive) return;

            int playerRate = Session.Parameters.AIEncirclePlayerRate;

            foreach (var faction in Session.Current.Scenario.PlayerFactions) 
            {
                if (!adjacentTo(faction) || IsFriendly(faction) || this == faction) continue;
                
                if (GameObject.Random(1000) < playerRate && GameObject.GetChance(faction.ArchitectureCount))
                {
                    if (GetEncircleFactionList(faction, true) == null) continue;

                    foreach (var architecture in Architectures)
                    {
                        if (architecture.Fund > 120000 + architecture.AbundantFund)
                        {
                            Encircle(architecture, faction);
                            return;
                        }
                    }
                }
            ...
            {
                var factions = GetAdjecentHostileFactions();
                ...
                var target = factions[(factions.Count - 1) * rank / 100];
        ...
        {
            var scope = new List<Architecture>();
            ...
            scope.Sort((a, b) => b.Population.CompareTo(a.Population));
        ...
        {
            foreach (var legion in StaticMethods.GetRandomList(Legions))
            {
                legion.AI();
            }
        }
        ...
        private void AITrainChildren()
        {
            if (GameObject.Random(90 / Session.Current.Scenario.Parameters.DayInTurn) == 0)
            {
                foreach (Person p in this.Children)
                {
                    ...
                    if (Session.Current.Scenario.IsPlayer(this) && (p.Father == Leader || p.Father == null || p.Mother == Leader || p.Mother == null)) continue;
        ...
        private void SetSectionAIDetailPinAtPlayer()
        {
            ...
            foreach (var section in Sections)
            {
                ...
                var playerFactions = Session.Current.Scenario.PlayerFactions;
                bool assigned = false;
        ...
        public void ChangeCapital(Architecture newCapital)
        {
            if (this.Capital != newCapital)
            {
                ...
                eventManager.Publish(new InitiativeChangeCapitalEvent(this, capital, Capital));
        ...
        public void AfterChangeLeader(Faction newFaction, GameObjectList candidates, Person oldLeader, Person newLeader)
        {
            foreach (var architecture in Architectures)
            {
                foreach (var person in architecture.GetConcubines())
                {
        ...
        public Faction ChangeLeaderAfterLeaderDeath()
        {
            ...
            if (person2 == null && Session.GlobalVariables.PermitFactionMerge && !IsAlien)
            {
                ...
                foreach (var relation in GetDiplomaticRelations())
                {
                ...
                if (diplomaticFaction != null)
                {
                    ...
                    foreach (var treasure in leader.Treasures)
                    {
            ...
            if (person2 != null)
            {
                ...
                foreach (var treasure in leader.Treasures.ToList())
                {
                    leader.LoseTreasure(treasure);
                    Leader.ReceiveTreasure(treasure);
                }
        ...
        public void CheckLeaderDeath(Person leader)
        {
            if ((((((leader.LocationArchitecture != null) && (leader.LocationArchitecture.BelongedFaction == this.Leader.BelongedFaction)) 
                ...
                && Session.Current.Scenario.IsPlayer(this)))
            {
                eventManager.Publish(new AfterCatchLeaderEvent(leader, this));
            }
        }

        public void ClearRouteways()
        {
            while (RoutewayCount > 0)
            {
                Session.Current.Scenario.RemoveRouteway(Routeways[0]);
            }
        }
        ...
        public List<Military> TransferingMilitaries { get; set; } = new();
        ...
        private void AIAppointMayor()
        {
            ...
            foreach (var architecture in Architectures)
            {
                if (architecture.BelongedSection.AIDetail.AutoRun && architecture.AppointMayorAvail())
                {
                    var persons = architecture.GetAIMayorCandicates();
                    var targetPerson = persons[0];

                    architecture.MayorID = targetPerson.ID;
                    architecture.AppointMayor(targetPerson);
                    architecture.MayorOnDutyDays = 0;
                }
            }
        }
      ...
      //  private Dictionary<PersonGeneratorType, int> types = new Dictionary<PersonGeneratorType, int>();
        ...
        public void Destroy()
        {
            ...
            eventManager.Publish(new FactionDestoryEvent(this));
            ...
            */
            ...
            Session.Current.Scenario.RemoveDiplomaticRelationByFactionID(ID);
            Session.Current.Scenario.Factions.Remove(ID);
        ...
        public List<Faction> GetHostileFactions()
        {
            ...
            {
                if (faction != this && !IsFriendly(faction))
                {
                    result.Add(faction);
                }
            }
        ...
        public List<Faction> GetOtherFactions()
        {
            var result = new List<Faction>();
        ...
        public List<Faction> GetUnderZeroDiplomaticRelationFactions()
        {
            var result = new List<Faction>();
            ...
            foreach (var relation in GetDiplomaticRelations())
            {
                ...
                {
                    result.Add(relation.RelationFaction2);
                }
                else
                {
                    result.Add(relation.RelationFaction1);
                }
            }
            return result;
        }
        ...
        public void HandleForcedChangeCapital()
        {
            ...
            if (Architectures.Count != 1)
            {
                ...
                if (Architectures.Count == 2)
                {
                    foreach (var architecture2 in Architectures)
                    {
                        if (architecture2 != Capital)
                        {
                            Capital = architecture2;
                            eventManager.Publish(new ForcedChangeCapitalEvent(this, capital, Capital));
        ...
        */
        ...
        {
            if (Capital != null && (Session.MainGame.mainGameScreen.LoadScenarioInInitialization || Session.Current.Scenario.Date.Day == 1 || SectionCount == 0))
            {
                if (Session.MainGame.mainGameScreen.LoadScenarioInInitialization || (Session.Current.Scenario.Date.Month % 3 == 1))
                {
                    ...
                    // ToList浅拷贝防止修改原建筑列表
                    BuildSectionByArchitectureList(Architectures.ToList());
                    foreach (var section in Sections)
                    {
        ...
        */
        ...
        {
            Routeways.Remove(routeway);
            Session.Current.Scenario.Routeways.Remove(routeway.ID);
        ...
        public List<Faction> AdjecentFactionList
        {
            get
            {
               /* FactionList list = new FactionList();
                ...
                return list;
                */
                return GetAdjecentFactions();
            }
        }

        public List<Faction> GetAdjecentFactions()
        {
            var result = new List<Faction>();

            foreach (var faction in Session.Current.Scenario.Factions.Values)
            {
                if (adjacentTo(faction))
                {
                    result.Add(faction);
                }
            }
        ...
        public List<Faction> GetAdjecentHostileFactions()
        {
            var result = new List<Faction>();

            foreach (var faction in Session.Current.Scenario.Factions.Values)
            {
                if (adjacentTo(faction) && IsHostile(faction))
                {
                    result.Add(faction);
                }
            }
        ...
        private bool hasNonFriendlyFrontline
        {
            get
            {
                ...
                foreach (var i in Architectures)
                {
                    ...
                    {
                        if (j.BelongedFaction == null || j.BelongedFaction.ID == ID) continue;

                        if (Session.Current.Scenario.GetDiplomaticRelation(j.BelongedFaction.ID, ID) < relationThreshold)
                        {
                            return true;
                        }
                    }
        ...
        public List<Faction> GetEncircleFactionList(Faction target, bool simulate) 
        {
            var encircleList = new List<Faction>();

            foreach (var f in Session.Current.Scenario.Factions.Values)
            {
                ...
                {
                    if (((Session.Current.Scenario.GetDiplomaticRelation(target.ID, f.ID) +
        ...
        {
            var encircleList = GetEncircleFactionList(target, false);

            if (encircleList.Count == 0) return;
            ...
            foreach (var i in encircleList)
            {
                foreach (var j in encircleList)
                {
                    if (i != j)
                    {
                        if (Session.Current.Scenario.GetDiplomaticRelationTruce(i.ID, j.ID) < 180)
                        {
                            Session.Current.Scenario.SetDiplomaticRelationTruce(i.ID, j.ID, 180);
                        }
                    }
                }
        ...
        public void Encircle(Architecture encircler, Faction toEncircle)
        {
            ...
            int relation = Session.Current.Scenario.GetDiplomaticRelation(ID, toEncircle.ID);

            int relationOffset = relation > -Session.GlobalVariables.FriendlyDiplomacyThreshold ? -100 : -50;
            Session.Current.Scenario.ChangeDiplomaticRelation(ID, toEncircle.ID, relationOffset);
            toEncircle.Leader.AdjustRelation(Leader, -18f, -6);
            Leader.AdjustRelation(toEncircle.Leader, -6f, -2);

            //处理所有势力和被声讨方的关系
            int relationThreshold = Session.GlobalVariables.FriendlyDiplomacyThreshold / 2;
            foreach (var diplomaticRelation in toEncircle.GetDiplomaticRelations())
            {
                if (diplomaticRelation.Relation < relationThreshold)
                {
                    diplomaticRelation.Relation -= 100;
                }
            }
            //加入包围圈判定
        ...
        {
            var result = Session.Current.Scenario.GetDiplomaticRelationListByFactionID(ID);
        ...
        private void ResetFriendlyDiplomaticRelations()
        {
            if (!IsAlien)
            {
                ...
                foreach (var relation in GetDiplomaticRelations())
                {
                    var opposite = relation.GetDiplomaticFaction(ID);

                    if (opposite != null && relation.Relation >= -relationThreshold && opposite.IsAlien)
                    {
            ...
            {
                foreach (var i in GetDiplomaticRelations())
                {
                    Faction opposite = i.GetDiplomaticFaction(ID);
            ...
            if ((this.Leader.StrategyTendency != PersonStrategyTendency.维持现状))
            {
                // Break Relations
                int relationThreshold = Session.GlobalVariables.FriendlyDiplomacyThreshold;
                var nonFriendlyFactions = new List<Faction>();

                foreach (var faction in Session.Current.Scenario.Factions.Values)
                {
                    if (adjacentTo(faction) && Session.Current.Scenario.GetDiplomaticRelation(ID, faction.ID) < relationThreshold)
                    {
                        nonFriendlyFactions.Add(faction);
                    }
                }
                
                int totalPower = 0;
                var nearbyFactions = GetAdjecentFactions();

                foreach (var faction in nearbyFactions)
                {
                    if (Session.Current.Scenario.GetDiplomaticRelation(ID, faction.ID) < relationThreshold)
                    {
                ...
                foreach (var faction in nearbyFactions)
                {
                    int truce = Session.Current.Scenario.GetDiplomaticRelationTruce(ID, faction.ID);
                    int relation = Session.Current.Scenario.GetDiplomaticRelation(ID, faction.ID);

                    if (truce > 0) continue;

                    int unAmbition = 4 - Leader.Ambition;
                    if (nonFriendlyFactions.Count == 0 && faction.Power < Power)
                    {
                        power = faction.Power;
                        toBreak = faction;
                    }
                    else if (Power > totalPower * ((unAmbition * unAmbition + (Leader.Calmness - Leader.Braveness) / 4) * 0.2 + 0.6) 
                            && relation >= relationThreshold)
                    {
                        float ratio = (float)Power / faction.Power;
                        if (GameObject.GetChance((int)((ratio - 1) * Leader.Ambition * 10)))
                        {
                            power = faction.Power;
                            toBreak = faction;
                        }
                ...
                if (toBreak != null)
                {
                    Session.Current.Scenario.SetDiplomaticRelation(ID, toBreak.ID, 0);

                    Leader.DecreaseKarma(5);
                ...
                foreach (var rel in GetDiplomaticRelations())
                {
                    ...
                    foreach (DiplomaticRelation i in GetDiplomaticRelations())
                    {
        ...
        {
            var architectures = Session.Current.Scenario.GetHighViewingArchitecturesByPosition(position);

            foreach (var architecture in architectures)
            {
                if (architecture.BelongedFaction != null && !IsFriendly(architecture.BelongedFaction))
                {
                    return RoutewayWorkForce / 2;
                }
            }
            return 0;
        }
            ...
            if (Session.Current.Scenario.IsPlayer(this))
            {
                if (this.WantControl)
                {
                    if (!Session.Current.Scenario.Threading)
                    {
                        ...
                        eventManager.Publish(new GetControlEvent(this));
        ...
        private void SetSectionAIDetail_OLD()
        {
            ...
            if (SectionCount <= 1)
            {
                ...
                if (ArmyScale >= num5 * 30)
                {
                    ...
                    foreach (Faction faction2 in StaticMethods.GetRandomList(Session.Current.Scenario.Factions.Values.ToList()))
                    {
                ...
                foreach (var faction2 in StaticMethods.GetRandomList(GetUnderZeroDiplomaticRelationFactions()))
                {
            ...
            if (UpgradingTechnique >= 0)
            {
                ...
                if (UpgradingDaysLeft <= 0)
                {
                    if (Session.Current.Scenario.GameCommonData.AllTechniques.TryGetValue(UpgradingTechnique, out var technique))
                    {
                        ...
                        eventManager.Publish(new TechniqueFinishedEvent(this, technique));
        ...
        public void UpgradeTechnique(Technique technique, Architecture architecture)
        {
            ...
            eventManager.Publish(new UpgradeTechniqueEvent(this, technique, architecture));
        }
        ...
        public int feiziCount()
        {
            ...
            foreach (var architecture in Architectures)
            {
                num += architecture.GetConcubines().Count;
            }
            return num;
        }

        public List<Person> GetFactionConcubines()
        {
            var result = new List<Person>();

            foreach (var architecture in Architectures)
            {
                result.AddRange(architecture.GetConcubines());
            }
        ...
        public List<Military> Militaries
        {
            get
            {
                ...
                foreach (var troop in Troops)
                {
                    ...
                    var targetTroop = army.ShelledMilitary == null ? army : army.ShelledMilitary;
                    result.Add(targetTroop);
                }
        ...
        public int TransferingMilitaryCount => TransferingMilitaries.Count;
        ...
        public int PersonCount
        {
            ...
            get
            {
                ...
                foreach (var architecture in Architectures)
                {
                    ...
                    result += architecture.GetMovingPersons().Count;
                }
                foreach (var troop in Troops)
                {
                    result += troop.PersonCount;
                }
        ...
        public int TroopCountExcludeTransport => Troops.Count(x => !x.IsTransport);
        ...
        public List<Information> GetAllInformationList()
        {
            ...
            foreach (var architecture in Architectures)
            {
                result.AddRange(architecture.Informations);
            }
        ...
        public bool HasInformation() => GetAllInformationList().Count > 0;
